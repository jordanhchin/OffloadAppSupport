...
printf 'backslash filename\n' > "$SOURCE/name\\with-backslash"
...
assert_file "$DESTINATION/name\\with-backslash"
assert_file "$DESTINATION/"$'name\nwith-newline'
...
assert_file "$SOURCE/name\\with-backslash"
assert_file "$SOURCE/"$'name\nwith-newline'
...
check_app_migration_freshness "$MIGRATION_BACKUP" || fail "new migration backup was not current: $APPOFFLOAD_ERROR"
[ "$LAST_MIGRATION_FRESHNESS" = "current" ] || fail "freshness status was not current"
update_app_migration_backup "$MIGRATION_BACKUP" || fail "current backup update failed: $APPOFFLOAD_ERROR"
[ "$LAST_MIGRATION_UPDATED" -eq 0 ] || fail "current backup was duplicated"
printf 'updated app state\n' >> "$MIGRATION_EXTERNAL/state.db"
check_app_migration_freshness "$MIGRATION_BACKUP"; FRESHNESS_STATUS=$?
[ "$FRESHNESS_STATUS" -eq 2 ] && [ "$LAST_MIGRATION_FRESHNESS" = "stale" ] || fail "modified offload was not detected as stale"
update_app_migration_backup "$MIGRATION_BACKUP" || fail "stale backup update failed: $APPOFFLOAD_ERROR"
UPDATED_MIGRATION_BACKUP="$LAST_MIGRATION_BACKUP"
[ "$LAST_MIGRATION_UPDATED" -eq 1 ] && [ "$UPDATED_MIGRATION_BACKUP" != "$MIGRATION_BACKUP" ] || fail "stale backup was not refreshed separately"
assert_dir "$MIGRATION_BACKUP"
assert_dir "$UPDATED_MIGRATION_BACKUP"
check_app_migration_freshness "$UPDATED_MIGRATION_BACKUP" || fail "refreshed backup was not current: $APPOFFLOAD_ERROR"
CORRUPT_MIGRATION_BACKUP="$TEST_ROOT/Corrupt.appbackup"
/usr/bin/ditto "$UPDATED_MIGRATION_BACKUP" "$CORRUPT_MIGRATION_BACKUP" || fail "could not make corrupt-backup fixture"
printf 'tampered\n' >> "$CORRUPT_MIGRATION_BACKUP/payload/App/Migrator.app/Contents/MacOS-data"
check_app_migration_freshness "$CORRUPT_MIGRATION_BACKUP"; FRESHNESS_STATUS=$?
[ "$FRESHNESS_STATUS" -eq 1 ] || fail "corrupt backup was treated as merely stale"
assert_contains "$APPOFFLOAD_ERROR" "checksum verification failed"
"$ROOT/bin/appoffload" app backups check "$UPDATED_MIGRATION_BACKUP" > "$TEST_ROOT/check-current.txt" || fail "CLI freshness check rejected a current backup"
assert_contains "$(/bin/cat "$TEST_ROOT/check-current.txt")" "CURRENT:"
printf 'newly discovered item\n' > "$MIGRATION_SUPPORT/new-item.db"
check_app_migration_freshness "$UPDATED_MIGRATION_BACKUP"; FRESHNESS_STATUS=$?
[ "$FRESHNESS_STATUS" -eq 2 ] || fail "new associated file did not stale the backup"
"$ROOT/bin/appoffload" app backups check "$UPDATED_MIGRATION_BACKUP" > "$TEST_ROOT/check-stale.txt"; FRESHNESS_STATUS=$?
[ "$FRESHNESS_STATUS" -eq 3 ] || fail "CLI stale check did not return exit code 3"
assert_contains "$(/bin/cat "$TEST_ROOT/check-stale.txt")" "STALE:"
"$ROOT/bin/appoffload" app batch --target "$TARGET2" "$MIGRATION_APP" "$TEST_ROOT/Not Installed.app" > "$TEST_ROOT/batch.txt" 2>&1; BATCH_STATUS=$?
[ "$BATCH_STATUS" -eq 1 ] || fail "CLI batch did not report partial failure"
assert_contains "$(/bin/cat "$TEST_ROOT/batch.txt")" "1 backed up, 1 failed"
...
check_app_migration_freshness "$NETWORK_BACKUP" || fail "network backup was not current: $APPOFFLOAD_ERROR"
